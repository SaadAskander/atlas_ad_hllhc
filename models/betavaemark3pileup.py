...
ggf_pileup_signal_acceptance = []
vbf_pileup_signal_acceptance = []
hs_pileup_signal_acceptance = []

for i in range(0,41):
    print(f"Pileup_ upper = {i * 0.1}")
    model = BetaVAEMark3(latent_dim = 256) 
    beta = np.ones(100) * 1e-1
    beta[:50] = np.linspace(0,49,50) * 1e-1/50
    tools.train(project_name = "BetaVAEMark3Pileup", model = model,
            weights_directory_path=f"/home/xzcapask/atlas_ad_hllhc/data/model_data/BetaVAEMark3/pileup/{i}/weights",
            training_losses_directory_path=f"/home/xzcapask/atlas_ad_hllhc/data/model_data/BetaVAEMark3/pileup/{i}/training_losses",
            beta = beta, pileup_cutoff = 0.1 * i)


    model = BetaVAEMark3(latent_dim = 256,log_clamping= False).to(device)
    model.load_state_dict(torch.load(f"/home/xzcapask/atlas_ad_hllhc/data/model_data/BetaVAEMark3/pileup/{i}/weights/BetaVAEMark3Pileup_weights_epoch100.pth"))
    kl_ggf_signal,kl_vbf_signal,kl_hs_signal, = tools.test(model = model, return_kl_scores = True, 
           signal_acceptance_directory = f"/home/xzcapask/atlas_ad_hllhc/data/model_data/BetaVAEMark3/pileup/{i}/signal_acceptance_rates",
           phi_invariance_study_directory=f"/home/xzcapask/atlas_ad_hllhc/data/model_data/BetaVAEMark3/pileup/{i}/phi_invariance_study",
           latent_code_directory=f"/home/xzcapask/atlas_ad_hllhc/data/model_data/BetaVAEMark3/pileup/{i}/latent_vectors",
           testing_losses_directory=f"/home/xzcapask/atlas_ad_hllhc/data/model_data/BetaVAEMark3/pileup/{i}/testing_losses",pileup_cutoff = 0.1 * i)
    
    ggf_pileup_signal_acceptance.append(kl_ggf_signal)
    vbf_pileup_signal_acceptance.append(kl_vbf_signal)
    hs_pileup_signal_acceptance.append(kl_hs_signal)

np.save(file = "/home/xzcapask/atlas_ad_hllhc/data/model_data/BetaVAEMark3/pileup/ggf_pileup_signal_acceptance", arr = ggf_pileup_signal_acceptance)
np.save(file = "/home/xzcapask/atlas_ad_hllhc/data/model_data/BetaVAEMark3/pileup/vbf_pileup_signal_acceptance", arr = vbf_pileup_signal_acceptance)
np.save(file = "/home/xzcapask/atlas_ad_hllhc/data/model_data/BetaVAEMark3/pileup/hs_pileup_signal_acceptance", arr = hs_pileup_signal_acceptance)
