...
class BetaVAEMark6Encoder(nn.Module):
    """
    Beta VAE Mark 6 Encoder 
    """
    def __init__(self, latent_dim = 7):
        ...
        self.pool1 = nn.Conv2d(in_channels = 8, out_channels = 8, kernel_size = 2, stride = 2)
        self.pool2 = nn.Conv2d(in_channels = 16, out_channels = 16, kernel_size = (5,2), stride = (5,2))
        self.pool3 = nn.Conv2d(in_channels = 32, out_channels = 32, kernel_size = (5,2), stride = (5,2))
...
class BetaVAEMark6Decoder(nn.Module):
    """
    Beta VAE Mark 6 Decoder.
    """
    def __init__(self, latent_dim = 7):
        super().__init__()
...
class BetaVAEMark6(nn.Module):
    ...
    def __init__(self, latent_dim = 7):
        super().__init__()
        self.encoder = BetaVAEMark6Encoder(latent_dim= latent_dim)
        self.decoder = BetaVAEMark6Decoder(latent_dim=latent_dim)
...
model = BetaVAEMark6() 
beta = np.zeros(100)
tools.train(project_name = "BetaVAE6beta0", model = model,
            weights_directory_path="/home/xzcapask/atlas_ad_hllhc/data/model_data/BetaVAEMark6/weights",
            training_losses_directory_path="/home/xzcapask/atlas_ad_hllhc/data/model_data/BetaVAEMark6/training_losses", beta = beta)

tools.test(model = model,
           signal_acceptance_directory = "/home/xzcapask/atlas_ad_hllhc/data/model_data/BetaVAEMark6/signal_acceptance_rates",
           phi_invariance_study_directory="/home/xzcapask/atlas_ad_hllhc/data/model_data/BetaVAEMark6/phi_invariance_study",
           latent_code_directory="/home/xzcapask/atlas_ad_hllhc/data/model_data/BetaVAEMark6/latent_vectors",
           testing_losses_directory="/home/xzcapask/atlas_ad_hllhc/data/model_data/BetaVAEMark6/testing_losses")
